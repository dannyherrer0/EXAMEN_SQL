...
```
SELECT e.nombre AS empleado, s.nombre, d.nombre,
JOIN sucursal AS s ON s.id = e.sucursal,
JOIN sucursal AS m ON m.id = s.municipio,
JOIN sucursal AS m ON m.id = dep.id,
JOIN departamento AS d ON d.id = m.depid;
```
...
JOIN clientes AS m ON municipioid = c.municipio,
JOIN municipio AS m ON depid = m.depid,
JOIN departamento AS d ON paisid = p.nombre;
```
...
```
SELECT m.nombre AS municipio, AVG(e.salario)AS
FROM empleados AS e
JOIN sucursal AS s ON s.id = e.sucursalid
Join municipio AS m ON m.id = e.sucursalid
GROUP BY municipio
LIMIT 10;
```
...
```
SELECT d.nombre AS departamento SUM(c.cliente.id) AS total_clientes
FROM clientes AS c,
JOIN municipio AS m ON m.id = municipio,
JOIN departamento AS d ON dep.id = depid
GROUP BY d.id
ORDER BUY departamento
LIMIT 3;
```
...
```
SELECT e.nombre AS empresa, AVG(em.salario) AS total_salarios
FROM empleados AS em
JOIN sucursal AS s ON s.id = em.sucursualid
JOIN empresa AS e ON e.id  = s.empresaid
GROUP BY empresa
ORDER BY empresa
...
```
SELECT m.nombre AS municipio, AVG(e.municipio_id) AS total_empleados
FROM empleados AS e,
JOIN sucursal AS s ON s.id = e.sucursal,
JOIN municipio AS m ON m.id = s.municipio,
GROUP BY m.nombre,
ORDER BY total_empleados = 10,
LIMIT 10;
```
...
```
SELECT m.nombre FROM municipio AS m,
JOIN sucursal AS s ON s.municipioid = m.id,
WHERE id NOT IN 
(SELECT m.id FROM municipio as m);
